		...
		for (i=0; i<numeroClientes; i++)
		{
			if (FD_ISSET (socketCliente[i], &descriptoresLectura))
			{
				...
				if ((Lee_Socket (socketCliente[i], &estructura, sizeof(t_socket)) > 0)){
					printf ("\nEl cliente %d mando %s %s. Hora de atenderlo\n", i+1, estructura.campo, estructura.valor);
					//crearLista(estructura.lista);
					crearLista(&lista);
					t_dato registros;
					while (feof(archivo) == 0)
					{
						fgets(palabra,200,archivo);
						strcpy(registros.item_id,strtok(palabra,delimitador));
						strcpy(registros.articulo,strtok(NULL,delimitador));
						strcpy(registros.producto,strtok(NULL,delimitador));
						strcpy(registros.marca,strtok(NULL,delimitador));
						...
						{
							if(strcmp(estructura.valor,registros.item_id) == 0){
								insertar(&lista,&registros);
						...
						else
						{
							...
							{
								if(strcmp(estructura.valor,registros.producto) == 0){
									insertar(&lista,&registros);
								}
							}
							else
							{
								...
								{
									if(strcmp(estructura.valor,registros.articulo) == 0){
										insertar(&lista,&registros);
									}
								}
								else
								{
									if(strcmp(estructura.valor,registros.marca) == 0){
										insertar(&lista,&registros);
									}
								}
							}
